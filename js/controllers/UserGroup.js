...
myApp.controller('UserGroupCtrl', function ($scope, $rootScope) {
    ...
    $scope.mapGroupAndRoleViewObject = function () {
        // for (var i = 0; i < $scope.userGroups.length; i++) {     debugger
        ...
        //            { $scope.userGroups[i].objRole =
        // angular.copy($scope.getRoleView("Disable"));         }             else {
        //             $scope.userGroups[i].objRole =
        // angular.copy($scope.getRoleView("View"));             }     }     else{
        ...
        for (var i = 0; i < $scope.userGroups.length; i++) {
            //loop role system
            for(var j=0;j<$scope.itemRole.length;j++ ){
                $scope.setPropertyRole($scope.itemRole[j].name);
            }
        }
        debugger
    }

    $scope.setPropertyRole = function (nameRole) {
        for (var i = 0; i < $scope.userGroups.length; i++) {
            if ($scope.userGroups[i].objPermissions.nameRole) {
                if ($scope.userGroups[i].objPermissions.nameRole == "crud") {
                    $scope.userGroups[i].objRole.nameRole = angular.copy($scope.getRoleView("Modify"));
                } else if ($scope.userGroups[i].objPermissions.nameRole == "----") {
                    $scope.userGroups[i].objRole.nameRole = angular.copy($scope.getRoleView("Disable"));
                } else {
                    $scope.userGroups[i].objRole.nameRole = angular.copy($scope.getRoleView("View"));
                }
            }
        }
    ...
    $scope.groupFromStringToObject = function () {
        ...
        var newPermission = '"';
        for (var i = 0; i < $scope.userGroups.length; i++) {
            //Loop role reset string permiassion
            newPermission = '"';
